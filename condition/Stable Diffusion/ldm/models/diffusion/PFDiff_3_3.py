...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        seq_next_next_next = list(time_range[3:]) + [-1] + [-2] + [-3]
        seq_next_next_next_next = list(time_range[4:]) + [-1] + [-2] + [-3] + [-4]

        xs = [img]

        et_list = []
        for index, (i_t, j_t, k_t, l_t, m_t) in enumerate(zip(seq, seq_next, seq_next_next, seq_next_next_next, seq_next_next_next_next)):
            if index != 0 and (index-1) % 4 !=0:
                continue
            i = total_steps - index - 1
            j = total_steps - index - 2
            k = total_steps - index - 3
            l = total_steps - index - 4
            m = total_steps - index - 5
            ...
            if len(et_list) == 0:
                ...
                next_next_t = torch.full((b,), k_t, device=device, dtype=torch.long)
                next_next_next_t = torch.full((b,), l_t, device=device, dtype=torch.long)
                xt = xs[-1]


                if l_t == -1:
                    #  The default choice is to try adding PFDiff_2_2 first, as it performs better in fewer steps.
                    et_ = et_list[-1]
                    xt_next = self.phi_any(et_, xt, i, j)

                    et_next = self.my_model(xt_next, cond, next_t,
                        score_corrector=score_corrector,
                        corrector_kwargs=corrector_kwargs,
                        unconditional_guidance_scale=unconditional_guidance_scale,
                        unconditional_conditioning=unconditional_conditioning)

                    xt_next_next_next = self.phi_any(et_next, xt, i, l)
                    et_list.append(et_next)
                    xs.append(xt_next_next_next)
                    break
                ...
                et_= et_list[-1]
                x_next_next_next = self.phi_any(et_, xt, i, l)

                et_next_next_next = self.my_model(x_next_next_next, cond, next_next_next_t,
                                                score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning)
                        
                xt_next_next_next_next = self.phi_any(et_next_next_next, xt, i, m)
                et_list.append(et_next_next_next)

                xs.append(xt_next_next_next_next)
